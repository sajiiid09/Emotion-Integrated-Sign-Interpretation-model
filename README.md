...
- **m**: presentation mode (hide debug counters)
- **h**: show quick help overlay
...
- Logs (when enabled via `BRAIN_LOG_ENABLED=1`) rotate in `logs/brain_events.jsonl`; cache is used when enabled to reduce repeated Gemini calls.
