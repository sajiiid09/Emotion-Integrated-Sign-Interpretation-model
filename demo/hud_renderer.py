...
class HUDRenderer:
    ...
    def render(
        ...
        *,
        ...
        chip_x -= chip_w + 8
        gemini_label = "Gemini ON" if gemini_on else "Gemini OFF"
        gemini_color = self.theme.accent if gemini_on else self.theme.neutral
        _draw_chip(draw, (chip_x, chip_y, chip_x + chip_w, chip_y + chip_h), gemini_label, self.font_small, gemini_color)
        chip_x -= chip_w + 8
        api_label = "Key ✓" if api_key_present else "No Key"
        api_color = self.theme.success if api_key_present else self.theme.danger
        _draw_chip(draw, (chip_x, chip_y, chip_x + chip_w, chip_y + chip_h), api_label, self.font_small, api_color)
        ...
        sections = [
            ...
            ("Sentence", self._render_sentence, {"text": display_sentence, "last_word": last_word}),
        ...
        if show_help:
            help_card = help_text or "q: quit | c: clear | g: toggle Gemini | m: presentation | h: help"
            card_w = int(w * 0.6)
            card_x = (w - card_w) // 2
            card_y = self.margin
            card_h = self.font_small.size * 4
            draw.rounded_rectangle((card_x, card_y, card_x + card_w, card_y + card_h), radius=10, fill=(0, 0, 0, 180))
            draw.text((card_x + 12, card_y + 8), help_card, font=self.font_small, fill=self.theme.text_primary)
    ...
    def _render_sentence(
        ...
        last_word = payload.get("last_word")
        ...
        if last_word:
            height += self.font_small.size + 4
        if measure_only:
            return height
        for idx, line in enumerate(lines):
            draw.text((x1, y1 + idx * (self.font_body.size + 4)), line, font=self.font_body, fill=self.theme.text_primary)
        if last_word:
            draw.text((x1, y1 + len(lines) * (self.font_body.size + 4) + 4), f"শেষ স্থিত শব্দ: {last_word}", font=self.font_small, fill=self.theme.text_muted)
        return height
