...
class GeminiClient:
    ...
    def generate(self, prompt: BuiltPrompt) -> Tuple[str, dict[str, object]]:
        ...
        for attempt in range(attempts):
            ...
            try:
                ...
                text = self._postprocess_bn(self._extract_text(response))
    ...
    def stream(self, prompt: BuiltPrompt) -> Iterator[str]:
        ...
        try:
            ...
            for chunk in stream:
                text = self._postprocess_bn(self._extract_text(chunk))
                if text:
                    yield text
        except Exception:
            return
    ...
    def _postprocess_bn(self, text: str) -> str:
        cleaned = text.strip()
        ...
        words = cleaned.split()
        if len(words) > self.cfg.max_response_words:
            cleaned = " ".join(words[: self.cfg.max_response_words]) + "..."
        return cleaned
